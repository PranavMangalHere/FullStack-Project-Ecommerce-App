...
const SigninSignUp = ({ setIsAuthenticated }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      let response;
      if (isSignUp) {
        response = await axios.post("http://localhost:5000/api/user/register", data);
        ...
        if (response.data.message) {
          ...
          // Check if setIsAuthenticated is passed and call it
          if (setIsAuthenticated) {
            setIsAuthenticated(true);
          } else {
            console.warn("setIsAuthenticated is not defined");
          }
